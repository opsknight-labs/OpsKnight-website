...
The v1.4 New Relic route authenticates with the integration ID and integration key. It does not verify the integration's optional signature secret or an `X-Signature` header. Keep the generated URL/key secret and use a trusted gateway if your policy requires an additional sender-verification layer.
