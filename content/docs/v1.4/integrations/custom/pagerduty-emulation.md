...
You can instead include `?integrationId=INTEGRATION_ID`, but the request must still provide the matching key in one of the supported locations above. `Authorization: Token token=…` and a `routing_key` query parameter are not accepted by the current v1.4 route.
