...
v1.3 does not provide native MFA, passkey/WebAuthn login, SAML, or email magic-link authentication. Enforce MFA through the configured OIDC provider or a trusted access proxy when required. The mobile platform-authenticator prompt is only a privacy overlay.
