...
OpsKnight v1.3 includes five dashboard templates:
...
## Important v1.3 limitations

- **Export as PDF** and **Share Dashboard** are visible but disabled in the current UI.
- Template previews are not saved until cloned.
- The edit toolbar can change the current browser view, but do not rely on layout edits being durable unless a reload confirms they were saved by your build. The dashboard API supports updating owned dashboards, while the current v1.3 viewer does not persist every edit-mode change.
...
| Layout changes disappear after reload      | See the v1.3 persistence limitation above; verify changes before relying on the layout.      |
| PDF/share is unavailable                   | Those menu items are disabled in v1.3. Use the supported analytics export where appropriate. |
