...
OpsKnight v1.3 ships as one Next.js App Router application backed by PostgreSQL. The same application serves the desktop UI, mobile routes under `/m`, public status pages, API routes, integration webhooks, and the internal scheduler. There is no separately deployed worker service and Redis is not a runtime dependency.
...
Authentication is implemented with NextAuth credentials and optional generic OIDC. OpsKnight v1.3 does not provide email magic-link authentication or SAML.
