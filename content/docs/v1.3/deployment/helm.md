...
Images built after `1.3.1` from this revision run `prisma migrate deploy` before starting the server. They retry migration failures and may run the packaged recovery helper between attempts. If migrations still fail, the container exits non-zero. Confirm the selected release includes this entrypoint behavior.
