...
Slack uses its own retry path in v1.4. Although a Slack breaker factory exists in source, the active Slack send paths do not use it, so do not assume Slack circuit state follows this page.
...
- there is no published v1.4 admin API for inspecting or resetting it.
