...
OpsKnight v1.4 resolves only when `status` (preferred) or `eventType` contains `resolve`, `resolved`, `close`, `closed`, `recover`, `recovered`, `ok`, or `up`. The common Splunk values `CLEAR` and `Cleared` are **not** recognized as recovery by the v1.4 normalizer.
...
Splunk Observability supports a provider-specific shared-secret mechanism, but OpsKnight v1.4's generic route expects an unprefixed raw-body HMAC-SHA256 digest in `X-Signature` or `X-Webhook-Signature`. Do not assume the two formats match.
...
Inspect the rendered `status` and `eventType`. `CLEAR`/`Cleared` is the known v1.4 mismatch; map it to `resolved` before forwarding.
